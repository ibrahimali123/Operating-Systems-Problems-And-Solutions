...
public class Directory {
	...
	public ArrayList<VFS_Visitor_DFS.File> files;
	public ArrayList<VFS_Visitor_DFS.Directory> subDirectories;
	...
	boolean recursev(String path, Visitor v) {
		...
		for (int i = 0; i < subDirectories.size(); i++) {
			if (subDirectories.get(i).Name.equals(name)) {
				return subDirectories.get(i).recursev(subPath, v);
			}
		}
	...
	public void fromJson(JSONObject ob) throws JSONException {
		...
		for (int i = 0; i < fileSize; i++) {
			...
			VFS_Visitor_DFS.File file = new File(obb.getString("name"),
					obb.getInt("size"), allocated);
			files.add(file);
		}
		...
		for (int i = 0; i < folderSize; i++) {
			VFS_Visitor_DFS.Directory directory = new Directory("", "");
